#include "pch.h"
#include "Graphics/Vulkan/Linux/Linux_Vulkan_Graphics.h"
...
bool Graphics_CreateGraphics(GfxHandle* pHandle, const GfxInitProps* pProps)
{
    *pHandle = NULL;
    ...
    Display* pDisplay = pProps->wndHandle->pDisplay;
    Window   window   = pProps->wndHandle->window;

    ASSERT_MSG(pDisplay && window, "Invalid window handle");
    ...
    result = pfn_vkCreateXlibSurfaceKHR(instance, &surfaceCreateInfo, NULL, &surface);
    if (result != VK_SUCCESS)
    {
    ...
    ASSERT(XGetWindowAttributes(pDisplay, window, &wAttribs));

    VkExtent2D extent = {wAttribs.width, wAttribs.height};
